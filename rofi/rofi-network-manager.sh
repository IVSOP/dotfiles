...
DIR="$HOME/.config/rofi/current-theme"
...
function initialization() {
	notify-send "Rofi network manager" "powering on" -t 1000
	source "$DIR/network-manager.conf" || exit # || source "${XDG_CONFIG_HOME:-$HOME/.config}/rofi/rofi-network-manager.conf" || exit
	{ RASI_DIR="$DIR/menu-theme.rasi" && [[ -f "$DIR/network-manager.rasi" ]]; } || exit # || { RASI_DIR="${XDG_CONFIG_HOME:-$HOME/.config}/i3/themes/current-theme/rofi-network-manager.rasi" && [[ -f "${XDG_CONFIG_HOME:-$HOME/.config}/i3/themes/current-theme/rofi-network-manager.rasi" ]]; } || exit
...
function notification() {
	[[ "$NOTIFICATIONS_INIT" == "on" ]] && notify-send -r "5" -u "normal" $1 "$2"
}
